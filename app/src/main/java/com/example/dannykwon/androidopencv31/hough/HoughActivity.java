package com.example.dannykwon.androidopencv31.hough;

import com.example.dannykwon.androidopencv31.R;
import com.example.dannykwon.androidopencv31.hough.line.HoughLine;
import com.example.dannykwon.androidopencv31.hough.line.Line;
import com.example.dannykwon.androidopencv31.hough.pref.PreferencesActivity;
import com.example.dannykwon.androidopencv31.hough.transform.HoughCircles2D;
import com.example.dannykwon.androidopencv31.hough.transform.HoughCircles3D;
import com.example.dannykwon.androidopencv31.hough.transform.HoughLineTransform;
import com.example.dannykwon.androidopencv31.hough.transform.HoughLines;
...
public class HoughActivity extends Activity implements CvCameraViewListener2 {
    ...
    private void drawBordersToMRgba() {
        if (viewMode == VIEW_MODE_OPENCV_LINES_HORIZON || orientation == 1) {
            Imgproc.line(matRgba, new Point(0, height / 2 - 1), new Point(width, height / 2 - 1), new Scalar(0, 255, 0), 1);
        } else {
            Imgproc.line(matRgba, new Point((2 * width) / 3 - 1, 0), new Point((2 * width) / 3 - 1, height), new Scalar(0, 255, 0), 1);
        }
    }
    ...
    private void openCVLines() {
        ...
        for (int j = 0; j < lines.cols(); j++) {
            ...
            //Draw the line
            Imgproc.line(tmp, start, end, new Scalar(255, 0, 0), 3);
        }
    ...
    private void openCVLinesWithHorizon() {
        ...
        if (i != null && i.x > 0 && i.x < width && i.y > 0 && i.y < height) {
            ...
            Imgproc.line(matRgba, leftLane.getStart(), i, new Scalar(255, 0, 0), 3);
            Imgproc.line(matRgba, rightLane.getStart(), i, new Scalar(255, 0, 0), 3);
        } else {
            //Draw the lines without horizon
            Imgproc.line(matRgba, leftLane.getStart(), leftLane.getEnd(), new Scalar(255, 0, 0), 3);
            Imgproc.line(matRgba, rightLane.getStart(), rightLane.getEnd(), new Scalar(255, 0, 0), 3);
        }
    ...
    private void openCVCircles() {
        ...
        for (int i = 0; i < circles.cols(); i++) {
            ...
            //Draw circle
            Imgproc.circle(matRgba, center, radius, new Scalar(0, 255, 0), 3);
        }
